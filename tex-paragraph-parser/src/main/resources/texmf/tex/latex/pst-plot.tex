...
\def\beginqp@dots{%
  ...
  /TheDot { gsave translate \psk@dotangle \psk@dotscale Dot grestore } def
  TheDot }
...
\def\psset@@tickstyle#1#2\@nil{%
  \ifx#1f\def\psk@tickstyle{0}\else
    \ifx#1t\def\psk@tickstyle{1}\else
      \ifx#1b\def\psk@tickstyle{-1}\else
...
\def\pst@hlabels#1#2{%
  \ifdim#1=\z@\else
    ...
    \ifnum\pst@cnta=\z@\else
      ...
      \fi
      \ifnum\psk@labels<3 \pst@@hlabels\fi
      \showoriginfalse
    \fi
  \fi}
...
\def\pst@@hlabels{%
  \vbox to\z@{%
    ...
    \fi
    \hbox to\z@{%
      \ifshoworigin\hbox to \z@{\hss\psk@Ox\hss}\fi
      \mmultido
        {\n=\psk@Ox+\psk@Dx}%
        {\pst@cnta}%
