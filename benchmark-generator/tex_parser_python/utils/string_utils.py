def split(string, delim, num, default=None):
    ...
    If str.split() returns more than <num>-many substrings, the result list
    will be truncated.
    If str.split() returns less than <num>-many substrings, the result list
    will be padded with the given default value, such that the length of the
    list is equal to <num>.

    Args:
        string (str): The string to split.
        delim (str): The delimiter.
        num (int): The expected length of substrings to return.
        default (str, optional): The default value to use when the length of
            the result list of str.split() is smaller than <num> and need to be
            padded.
    Returns:
