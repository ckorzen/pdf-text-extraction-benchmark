...
public class ArxivUnpacker {
   ...
   * The base directory.
   */
  protected static Path BASE_DIR = Paths.get("/nfs/raid1/arxiv/src/");
  ...
  protected static String PREFIX = "arXiv_src";
  ...
  protected static Filter<Path> SRC_CHUNKS_FILTER = new Filter<Path>() {
    ...
    public boolean accept(Path entry) throws IOException {
      ...
      return filename.startsWith(PREFIX) && filename.endsWith(".tar");
    }    
  };
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.println("Unpack src chunks");
    
    // Unpack the src chunks.
    List<Path> unpackedSrcArchives = unpackSrcChunks(TMP_DIR);
    
    System.out.println("Unpack src archives");
    
    // Unpack the archives.
    unpackSrcArchives(unpackedSrcArchives);
       
    
    System.out.println("Move..." + TMP_DIR + " " + BASE_DIR);
    // Move the files from tmp to base.
    PathUtils.move(TMP_DIR, BASE_DIR);
  }
  ...
  protected static List<Path> unpackSrcChunks(Path target) throws Exception {
    ...
    try (DirectoryStream<Path> dir = 
      ...
      while (dirItr.hasNext()) {
        Path next = dirItr.next();
        System.out.println(next);
        // Untar the chunk without deleting it.
        List<Path> files = untar(next, target, false);
  ...
  protected static void unpackSrcArchives(List<Path> inputFiles) 
    ...
    for (Path inputFile : inputFiles) {
      ...
      switch (extension) {
        ...
        case "pdf":
          PathUtils.move(inputFile, outputDirectory);
          break;
