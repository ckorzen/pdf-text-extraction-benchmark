...
public class ProcessEngine implements Closeable {
    ...
    // *********************************************
    // Changed by CK to be able to process single files.
    File[] files = null;   
    if (pdfDirectory.isDirectory()) {
      files = pdfDirectory.listFiles();
    } else if (pdfDirectory.isFile()) {
      files = new File[1];
      files[0] = pdfDirectory;
    }
		// File[] files = pdfDirectory.listFiles();
		// *********************************************
            ...
            for (final File currPdf : files) {
                try {
                    if (currPdf.getName().toLowerCase().endsWith(".pdf")) {
						...
						if (outputPath != null && outputPath.endsWith(".txt")) {
						  Utilities.writeInFile(outputPath, result.toString());
						  continue;
						}
