...
my $importDir = "output/"; #Directory where output should be stored
...
sub prep {
    ...
    my $targetPath = "$importDir/" . without_ext(basename($filePath)) . ".txt";
...
#     }
    ...
    my $textFile = "$importDir" . without_ext(basename($filePath)) . ".txt";
    print "CONVERT [OK] ";    
    my ($status, $msg) = filter($textFile);
    if ($status <= 0) {
        unlink($targetPath); 
