...
def rearrange_and_wdiff(in1, in2, normalize=True, ignore_cases=True, 
    ...
    for i, mapping in enumerate(insertions_word_mappings):             
        ...
        if run:
            ...
            runs_queue.put(((-run_length, run_spread), run, i))
    ...
    while not runs_queue.empty():
        ...
        if run:
            run_length = len(run)
            run_spread = run[-1].deletion.pos - run[0].deletion.pos
            runs_queue.put(((-run_length, run_spread), run, mapping_index))
...
def cmd_wdiff(in1, in2, normalize=True, ignore_cases=True):
    ...
    args.append('<(echo "%s")' % in1) 
    args.append('<(echo "%s")' % in2)
        
    return bash_command(args).stdout
