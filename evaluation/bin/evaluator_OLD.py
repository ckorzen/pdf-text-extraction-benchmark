...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for current_dir, dirs, files in os.walk(groundtruth_root):
            ...
            files = sorted([fi for fi in files if fi.startswith(prefix) \
                                       and fi.endswith(suffix)])
                                       
            for file in sorted(files):
                current, latest, best = self.evaluate_by_file(current_dir, file)
        ...
        if self.args.recap:
            print("-" * 133)
            print("%s%s" % (
                "Total: ".ljust(20), self.create_recap_string(latest_values)))
            print("-" * 133)
        else:
            print("-" * 133)
            print("%s%s" % (
                "Total: ".ljust(20), 
                self.create_comparison_string(current_values, latest_values, best_values)))
            print("-" * 133)
    ...
    def recap(self, actual_path, values):
        ...
        print("%s%s" % (os.path.basename(actual_path).ljust(20), recap))

    def evaluate_by_file(self, current_dir, file):
        ...
        if gt and actual:
            ...
            if self.args.recap:
                ...
                # Evaluate.
                result = self.evaluate_by_strings(gt_path, gt, actual_path, actual)
                ...
                if result:
                    ...
                    print("%s%s" % (
                        os.path.basename(actual_path).ljust(20), 
                        comparison_string))
    ...
    def serialize(self, values, path):
        ...
        with open(path, "a") as f:
            ...
            values_str.append(str(values.get('num_word_replaces', 0)))
    ...
    def create_comparison_string(self, current, latest, best):
        ...
        if current and isinstance(current, list):
            ...
            word_replaces = round(sum(x['num_word_replaces'] for x in current) / len(current), 1)
        else:
            ...
            word_replaces = current['num_word_replaces'] if current else 0
        ...
        if latest and isinstance(latest, list):
            ...
            latest_word_replaces = round(sum(x[7] for x in latest) / len(latest), 1)
        else:
            ...
            latest_word_replaces = latest[7] if latest else 0
        ...
        if best and isinstance(best, list):
            ...
            best_word_replaces = round(sum(x[7] for x in best) / len(best), 1)
        else:
            ...
            best_word_replaces = best[7] if best else 0
        ...
        word_replaces_delta = word_replaces - latest_word_replaces
                
        # Format the delta parts
        para_splits_delta_str   = "±0.0" if para_splits_delta == 0 else "%+.1f" % para_splits_delta
        para_merges_delta_str   = "±0.0" if para_merges_delta == 0 else "%+.1f" % para_merges_delta
        para_rearranges_delta_str = "±0.0" if para_rearranges_delta == 0 else "%+.1f" % para_rearranges_delta
        para_inserts_delta_str  = "±0.0" if para_inserts_delta == 0 else "%+.1f" % para_inserts_delta
        para_deletes_delta_str  = "±0.0" if para_deletes_delta == 0 else "%+.1f" % para_deletes_delta
        word_inserts_delta_str  = "±0.0" if word_inserts_delta == 0 else "%+.1f" % word_inserts_delta
        word_deletes_delta_str  = "±0.0" if word_deletes_delta == 0 else "%+.1f" % word_deletes_delta
        word_replaces_delta_str = "±0.0" if word_replaces_delta == 0 else "%+.1f" % word_replaces_delta
        
        # Compose the string.   
        parts = []
        parts.append(("%s" % para_splits).rjust(6))
        parts.append(("(%s)" % para_splits_delta_str).rjust(8))
        parts.append(("%s" % para_merges).rjust(6))
        parts.append(("(%s)" % para_merges_delta_str).rjust(8))
        parts.append(("%s" % para_rearranges).rjust(6))
        parts.append(("(%s)" % para_rearranges_delta_str).rjust(8))
        parts.append(("%s" % para_inserts).rjust(6))
        parts.append(("(%s)" % para_inserts_delta_str).rjust(8))
        parts.append(("%s" % para_deletes).rjust(6))
        parts.append(("(%s)" % para_deletes_delta_str).rjust(8))
        parts.append(("%s" % word_inserts).rjust(6))
        parts.append(("(%s)" % word_inserts_delta_str).rjust(8))
        parts.append(("%s" % word_deletes).rjust(6))
        parts.append(("(%s)" % word_deletes_delta_str).rjust(8))
        parts.append(("%s" % word_replaces).rjust(6))
        parts.append(("(%s)" % word_replaces_delta_str).rjust(8))
    ...
    def create_recap_string(self, latest):
        ...
        if latest and isinstance(latest, list):
            ...
            latest_word_replaces = sum(x[7] for x in latest) / len(latest)
        else:
            latest_para_splits   = latest[0] if latest else 0
            latest_para_merges   = latest[1] if latest else 0
            latest_para_rearranges = latest[2] if latest else 0
        ...
        # Compose the string.          
        parts = []
        parts.append(("%.1f" % latest_para_splits).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_para_merges).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_para_rearranges).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_para_inserts).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_para_deletes).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_word_inserts).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_word_deletes).rjust(6))
        parts.append("(±0.0)".rjust(8))
        parts.append(("%.1f" % latest_word_replaces).rjust(6))
        parts.append("(±0.0)".rjust(8))
