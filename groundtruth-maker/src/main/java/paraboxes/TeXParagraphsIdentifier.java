...
//    return paragraphs;
   ...
   * Identifies paragraphs in the given list of elements.
   */
  protected void processElements(List<Element> elements,
      List<TexParagraph> result) {
    affirm(elements != null, "No elements given.");
    ...
    for (Element element : elements) { 
      ...
      if (element instanceof Group) {
        List<TexParagraph> result2 = new ArrayList<>();
        processElements(((Group) element).elements, result2);
        if (result2.size() > 1) {
        ...
        processElements(((Option) element).elements, result2);
      ...
      if (element instanceof Command) {
        ...
        if (command.hasOptions()) {
          List<TexParagraph> result2 = new ArrayList<>();
          for (Option option : command.getOptions()) {
            processElements(option.elements, result2);
            if (result2.size() > 1) {
              result.addAll(result2);
            } else {
              paragraph.addTexElement(option);
            }
          }
        }
        if (command.hasGroups()) {
          for (Group group : command.getGroups()) {
            List<TexParagraph> result2 = new ArrayList<>();
            processElements(group.elements, result2);
            if (result2.size() > 1) {
              result.addAll(result2);
            } else {
              paragraph.addTexElement(group);
            }
          }
        }
      }
    ...
    // Don't forget to add last paragraph.
    if (paragraph.getNumTexElements() > 0) {
      result.add(paragraph);
    }
  }
